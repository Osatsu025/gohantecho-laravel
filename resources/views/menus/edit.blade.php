          ...
          <div class="max-w-xl mx-auto">
            <form method="POST" action="{{ route('menus.update', $menu) }}">
              ...
              <fieldset class="fieldset bg-base-200 border-base-300 rounded-box w-l border p-4">
                ...
                <input type="text" class="input w-lg" placeholder="タイトル" name="title" value="{{ old('title') ?? $menu->title }}">

                <label for="content" class="label">{{ __('Content') }}</label>
                <textarea name="content" id="content" class="textarea w-lg size-100" placeholder="{{ __('Write Ingredients and order of your original recipe or Other sites URL or Restaurants menu you ate') }}">{{ old('content') ?? $menu->content}}</textarea>
